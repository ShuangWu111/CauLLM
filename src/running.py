...
def validate(val_evaluator, config, best_metrics, best_value, epoch):
...
class SupervisedRunner(BaseRunner):
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            #可视化
            # ind = -2
            # if 0 in IDs and epoch_num==50:
            #     ind = IDs.index(0)
            #     print(ind)
            #     M_visual = M[ind].numpy()
            #     print(M_visual)
            #     print(targets[ind])
            
            predictions = self.model(X.to(self.device), padding_masks, M.to(self.device))
        ...
        if self.classification:
            ...
            if self.model.module.num_classes == 2:
