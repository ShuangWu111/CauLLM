...
from models.caugpt4ts import caugpt4ts
#from models.gpt4ts_lora import gpt4ts_lora
...
def main(config):
    ...
    device = torch.device('cuda:1' if (torch.cuda.is_available() and config['gpu'] != '-1') else 'cpu')
    ...
    if config['norm_from']:                                                                              
        ...
        logger.info("here")
        feature_tensor = torch.tensor(my_data.feature_df.values, dtype=torch.float32, device=device)
        feature_tensor[train_indices] = normalizer.normalize(feature_tensor[train_indices])
        
        #if not config['normalization'].startswith('per_sample'):
            # get normalizing values from training set and store for future use
            #norm_dict = normalizer.__dict__
            #with open(os.path.join(config['output_dir'], 'normalization.pickle'), 'wb') as f:
                #pickle.dump(norm_dict, f, pickle.HIGHEST_PROTOCOL) #保存参数，以便后续复用
    if normalizer is not None:
        if len(val_indices):
            val_tensor=torch.tensor(val_data.feature_df.values,dtype=torch.float32, device=device)
            val_tensor[val_indices] = normalizer.normalize(val_tensor[val_indices])
        if len(test_indices):
    ...
    model = caugpt4ts(config, my_data)
    #lora.apply_lora(model, r=8, lora_alpha=16, lora_dropout=0.1)

    if config['freeze']:
        for name, param in model.named_parameters():
            if name.startswith('padding_patch_layer') or name.startswith('enc_embedding') or name.startswith('gpt2.out_layer') or 'ln' in name or 'wpe' in name:
    ...
    for epoch in tqdm(range(start_epoch + 1, config["epochs"] + 1), desc='Training Epoch', leave=False):
        ...
        for k, v in aggr_metrics_train.items():
            ...
            print_str += '{}: {:8f} | '.format(k, v)
        print(print_str)
        print("Epoch runtime: {} hours, {} minutes, {} seconds\n".format(*utils.readable_time(epoch_runtime)))
        total_epoch_time += epoch_runtime
        avg_epoch_time = total_epoch_time / (epoch - start_epoch)
        avg_batch_time = avg_epoch_time / len(train_loader)
        avg_sample_time = avg_epoch_time / len(train_dataset)
        print("Avg epoch train. time: {} hours, {} minutes, {} seconds".format(*utils.readable_time(avg_epoch_time)))
        print("Avg batch train. time: {} seconds".format(avg_batch_time))
        print("Avg sample train. time: {} seconds".format(avg_sample_time))
        ...
        if (epoch == config["epochs"]) or (epoch == start_epoch + 1) or (epoch % config['val_interval'] == 0):
            aggr_metrics_val, best_metrics, best_value = validate(val_evaluator, config,
    ...
    validate_metrics_filepath = os.path.join(config["output_dir"], "validate_metrics_" + config["experiment_name"] + ".xls")
    train_metrics_filepath = os.path.join(config["output_dir"], "train_metrics_" + config["experiment_name"] + ".xls")
    book = utils.export_performance_metrics(validate_metrics_filepath, metrics, header, sheet_name="metrics")
    train_book = utils.export_performance_metrics(train_metrics_filepath, train_metrics, header, sheet_name="metrics")
    ...
    print('Best {} was {}. Other metrics: {}'.format(config['key_metric'], best_value, best_metrics))
    print('All Done!')

    total_runtime = time.time() - total_start_time
    print("Total runtime: {} hours, {} minutes, {} seconds\n".format(*utils.readable_time(total_runtime)))
