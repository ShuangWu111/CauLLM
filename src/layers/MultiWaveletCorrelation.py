...
class MultiWaveletTransform(nn.Module):
    ...
    def __init__(self, ich=1, k=8, alpha=16, c=128,
        ...
        self.L = L
        self.nCZ = nCZ
        self.Lk0 = nn.Linear(ich, c * k, dtype=torch.float32)
...
class MWT_CZ1d(nn.Module):
    def __init__(self,
        ...
        self.T0 = nn.Linear(k, k, dtype = torch.float32)
    ...
    def wavelet_transform(self, x):
        ...
        d = torch.matmul(xa.float(), self.ec_d.float())
        s = torch.matmul(xa.float(), self.ec_s.float())
        return d, s
    ...
    def evenOdd(self, x):
        ...
        B, N, c, ich = x.shape  # (B, N, c, k)
        assert ich == 2 * self.k
        x_e = torch.matmul(x.float(), self.rc_e.float())
        x_o = torch.matmul(x.float(), self.rc_o.float())
